...
function UndefinedPropertyError(path,property,object) {
   if(property !== prop_access(object,path)){
       return new Error("property '"+property+"' not existe for path '"+path+"', expected one of :[type,name]");
   }
}
...
console.log(UndefinedPropertyError("animal.type.name","chienne",{ animal: { type: { name: "chine"} } }));
